  ...
  template: `
    ...
    <form id="post-form" method="post" action="/add-post">
      ...
      <div class="form-button">
        <button class="btn btn-default" (click)="submit(user)">
          Отправить
        </button>
      </div>
    </form>
    ...
    <div *ngIf="done">
      ...
      <div>Имя: {{ receivedUser.text }}</div>
      <div>Возраст: {{ receivedUser.title }}</div>
      <div>Возраст: {{ receivedUser.author }}</div>
    </div>
